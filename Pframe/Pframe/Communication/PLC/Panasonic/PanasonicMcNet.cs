...
{
	public class PanasonicMcNet : NetDeviceBase
	{
		public PanasonicMcNet(DataFormat DataFormat = DataFormat.DCBA)
		{
		...
		public override CalResult<byte[]> ReadByteArray(string address, ushort length)
		{
			...
			{
				byte[] array = null;
				if (base.SendAndReceive(xktResult.Content, ref array, null, 5000) && (array != null && array.Length > 10))
				{
					ushort num = BitConverter.ToUInt16(array, 9);
                    result = num > 0 ? CalResult.CreateFailedResult<byte[]>(new CalResult()) : this.ExtractActualData(array, xktResult.Content[13] == 1);
                }
				else
		...
		{
			CalResult<MelsecMcDataType, int> xktResult = MelsecHelper.McAnalysisAddress(address, false);
		...
		public override CalResult WriteByteArray(string address, byte[] value)
		{
			...
			{
				byte[] array = null;
				if (base.SendAndReceive(xktResult.Content, ref array, null, 5000) && array != null)
				{
					ushort num = BitConverter.ToUInt16(array, 9);
					if (num > 0)
		...
		{
			CalResult<MelsecMcDataType, int> xktResult = MelsecHelper.PanasonicAnalysisAddress(address);
			CalResult<byte[]> result;
			if (!xktResult.IsSuccess)
			{
				result = CalResult.CreateFailedResult<byte[]>(xktResult);
