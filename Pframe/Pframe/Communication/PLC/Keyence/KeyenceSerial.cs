...
namespace Pframe.PLC.Keyence
{
    ...
    /// 基恩士串口协议
    /// </summary>
    public class KeyenceSerial : SerialDeviceBase
    {
        ...
        public string Read(string address, LinkDataType tpye = LinkDataType.None)
        {
            address = this.AddressHandle(address);
            string string_ = "RD " + address + this.GetDataType(tpye) + "\r";
        ...
        private string Read(string address, int count, LinkDataType tpye = LinkDataType.None)
        {
            address = this.AddressHandle(address);
        ...
        public bool Write(string address, string value, LinkDataType tpye = LinkDataType.None)
        {
            address = this.AddressHandle(address);
        ...
        private bool Write(string address, int count, List<string> value, LinkDataType tpye = LinkDataType.None)
        {
            address = this.AddressHandle(address);
        ...
        private string GetDataType(LinkDataType type)
        {
        ...
        private bool GetResult(string value, ref string relt)
        {
            byte[] bytes = null;
            bool result;
            if (result = base.SendAndReceive(this.encoding.GetBytes(value), ref bytes, 0))
                relt = this.encoding.GetString(bytes);
            return result;
        }
