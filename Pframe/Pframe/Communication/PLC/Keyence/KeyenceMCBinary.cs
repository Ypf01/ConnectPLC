...
namespace Pframe.PLC.Keyence
{
    ...
    /// 基恩士MC二进制通信类
    /// </summary>
	public class KeyenceMCBinary : NetDeviceBase
	{
		public KeyenceMCBinary(DataFormat DataFormat = DataFormat.DCBA)
		{
			...
			else
			{
				...
				if (base.SendAndReceive(xktResult.Content, ref array, message, 5000) && (array != null && array.Length != 0))
				{
					try
					{
						...
						return this.ExtractActualData(array, xktResult.Content[13] == 1);
					}
					catch (Exception)
					{
						return CalResult.CreateFailedResult<byte[]>(new CalResult());
		...
		public override CalResult<bool[]> ReadBoolArray(string address, ushort length)
		{
			...
			else
			{
				...
				{
					result = CalResult.CreateSuccessResult<bool[]>((from m in xktResult2.Content
					select m == 1).Take((int)length).ToArray<bool>());
				}
			}
			return result;
		}
		...
		private CalResult<byte[]> BuildWriteCommand(string address, byte[] value, byte networkNumber = 0, byte networkStationNumber = 0)
		{
			...
			else
			{
				...
				{
					num = value.Length;
                    value = KeyenceHelper.TransBoolArrayToByteData(value);
				}
