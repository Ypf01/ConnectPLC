...
{
	public class ModbusAsciiOverTcp : ModbusAsciiOverTcpMessage
	{
		public override byte[] ReadOutputStatus(byte slaveAddress, ushort startAddress, ushort numberOfPoints)
		{
			...
			byte[] value = null;
			if (base.SendAndReceive(sendByte, ref value, new ModbusASCIIMsg(modbusMessage), 5000))
			{
		...
		public override bool ForceCoil(byte slaveAddress, ushort startAddress, bool value)
		{
			...
			byte[] sendByte = base.BuildMessageFrame(modbusMessage);
			byte[] value2 = null;
			if (base.SendAndReceive(sendByte, ref value2, new ModbusASCIIMsg(modbusMessage), 5000))
			{
