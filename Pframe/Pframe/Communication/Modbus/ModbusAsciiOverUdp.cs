...
{
	public class ModbusAsciiOverUdp : ModbusAsciiOverTcpMessage
	{
		...
		public ModbusAsciiOverUdp(string ipAddress, int port, DataFormat dataFormat = DataFormat.ABCD)
		{
		...
		public override byte[] ReadOutputStatus(byte slaveAddress, ushort startAddress, ushort numberOfPoints)
		{
			...
			byte[] value = null;
			if (this.method_6(byte_, ref value, new ModbusASCIIMsg(modbusMessage), 5000))
			{
				modbusMessage.Response = base.GetCoreFromAsciiMsg(value);
		...
		public override bool ForceCoil(byte slaveAddress, ushort startAddress, bool value)
		{
			...
			byte[] byte_ = base.BuildMessageFrame(modbusMessage);
			byte[] value2 = null;
			if (this.method_6(byte_, ref value2, new ModbusASCIIMsg(modbusMessage), 5000))
			{
				modbusMessage.Response = base.GetCoreFromAsciiMsg(value2);
				if (base.ValidateResponse(modbusMessage))
				{
					return true;
				}
		...
		private bool method_6(byte[] byte_1, ref byte[] byte_2, IMessage imessage_0 = null, int int_7 = 5000)
		{
			...
			try
			{
				...
				socket.ReceiveTimeout = int_7;
				IPEndPoint ipendPoint = new IPEndPoint(IPAddress.Any, 0);
				EndPoint endPoint = ipendPoint;
				socket.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReceiveTimeout, int_7);
