...
{
	public class SiemensDeviceGroup : NodeClass, IXmlConvert
	{
		public SiemensDeviceGroup()
		{
			this.varList = new List<SiemensVariable>();
			base.Name = "组对象";
			base.Description = "作为一次批量的字节数据读取";
			this.DBNo = 1;
			this.Length = 10;
			this.StoreArea = SiemensStoreArea.DB存储区;
			this.Start = 0;
			base.NodeType = 100;
		...
		public SiemensStoreArea StoreArea { get; set; }
		...
		public override void LoadByXmlElement(XElement element)
		{
			...
			this.StoreArea = (SiemensStoreArea)Enum.Parse(typeof(SiemensStoreArea), element.Attribute("StoreArea").Value, true);
			this.Length = int.Parse(element.Attribute("Length").Value);
			this.Start = int.Parse(element.Attribute("Start").Value);
			this.DBNo = int.Parse(element.Attribute("DBNo").Value);
		...
		public override XElement ToXmlElement()
		{
			...
			xelement.SetAttributeValue("DBNo", this.DBNo);
		...
		public override List<NodeClassRenderItem> GetNodeClassRenders()
		{
			...
			nodeClassRenders.Add(new NodeClassRenderItem("DB号", (this.DBNo == 0) ? "无" : ("DB" + this.DBNo.ToString())));
		...
		public List<SiemensVariable> varList;
